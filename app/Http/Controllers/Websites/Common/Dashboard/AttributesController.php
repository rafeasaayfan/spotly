...
use App\Http\Requests\Websites\Common\Dashboard\Attributes\StoreAttributeRequest;
use App\Http\Requests\Websites\Common\Dashboard\Attributes\UpdateAttributeRequest;
...
class AttributesController extends BaseController
{
    ...
    {
        $query = Attribute::where('website_id', $this->website->id);
    ...
    {
        try {
            $query = Attribute::where('website_id', $this->website->id)->with(['values'])
                ->findOrFail($id);
    ...
    {
        try {
            $attribute = Attribute::where('website_id', $this->website->id)
                ->with(['values'])
                ->findOrFail($id);
            $attributes = config('attributes.ecommerce_attributes');
    ...
    public function destroy(Request $request)
    {
        try {
            $validated = $request->validate([
                'ids' => 'required|array',
                'ids.*' => 'integer|exists:attributes,id,website_id,' . $this->website->id,
            ]);
    ...
    {
        try {
            $attribute = Attribute::where('website_id', $this->website->id)->findOrFail($id);
