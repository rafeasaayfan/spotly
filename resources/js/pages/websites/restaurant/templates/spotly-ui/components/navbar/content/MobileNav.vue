            ...
            >
                ...
                <SheetHeader class="web-border-color flex border-b pb-3">
                    <Link :href="route('website.e-commerce.home')" class="cursor-pointer items-center gap-3">
                        ...
                        <span
                            v-if="!websiteNameAndLogo.dark_logo || !websiteNameAndLogo.light_logo"
                            class="web-text-active leading-non truncate text-base font-bold"
                        >
                    ...
                    <nav class="space-y-2">
                        <template v-for="item in navbarItems" :key="item.title">
                            <button
                                ...
                                class="web-text-body flex w-full items-center gap-x-3 rounded-lg bg-transparent px-3 py-2 text-sm font-medium hover:bg-[var(--bg_content_light)] dark:hover:bg-[var(--bg_content_dark)]"
                            ...
                            <Link
                                v-else
                                :href="item.href ?? ''"
                                class="web-text-body flex items-center gap-x-3 rounded-lg bg-transparent px-3 py-2 text-sm font-medium hover:bg-[var(--bg_content_light)] dark:hover:bg-[var(--bg_content_dark)]"
                                :class="activeNavStyle(item.href ?? '', 'web-bg-content-active web-text-active')"
                                :dir="page.props.lang == 'ar' ? 'rtl' : 'ltr'"
                            >
                                <component v-if="item.icon" :is="item.icon" class="h-5 w-5" />
