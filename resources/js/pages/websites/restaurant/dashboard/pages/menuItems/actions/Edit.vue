...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Edit',
        href: '/dashboard/menu-items/edit',
    },
];
...
const createInitialOptionGroup = () => {
    return props.menuGroups.map((group: any) => {
        const groupOptions = (props.data.options || [])
            .filter((opt: any) => opt.option_group_id === group.id)
            .map((opt: any) => {
                const isCustom = group.price_type === 'custom';

                return reactive({
                    name: opt.name,
                    name_ar: opt.name_ar,
                    price_delta: opt.price_delta ?? '',
                    is_increase: opt.is_increase,
                    is_custom_price_type: isCustom,
                    option_explain: opt.option_explain ?? '',
                    is_active: String(opt.is_active),
                });
            });
...
const form = useForm<{
    ...
    step: 1,
    restaurant_item_images: props.data.restaurant_item_images ?? [],
    category_id: props.data.category_id ?? '',
    name: props.data.name,
    price: props.data.price,
    discount_price: props.data.discount_price,
    group_options: reactive(createInitialOptionGroup()),
    is_active: props.data.is_active,
    is_discount: props.data.is_discount,
...
const nextStep = () => {
    ...
    form.post(route('website.restaurant.dashboard.menuItems.update', props.data.id), {
        onSuccess: () => {
            form.step = 2;
        },
        onError: (errors: any) => {
                ...
                <div class="flex flex-col gap-5 md:col-span-1">
                    ...
                    <div class="flex flex-wrap items-center justify-end gap-1.5 md:gap-1 lg:gap-2">
                        ...
                        <Button size="sm" :disabled="load" @click="nextStep()">
                            <span v-if="!load">{{ form.step < 2 ? 'Next' : 'Update' }}</span>
