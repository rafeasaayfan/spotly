                    ...
                    <div v-for="item in props.data.items" :key="item.id" class="bg-body flex flex-col gap-3 rounded-md p-2.5 lg:p-4">
                        <div class="border-muted flex w-full items-center gap-2 border-b pb-2">
                            <Image
                                ...
                                alt="Item Image"
                                class="max-h-16 min-h-16 max-w-16 min-w-16 rounded-full object-cover shadow"
                            />
                            <div
                                v-else
                                class="border-muted text-body-muted flex max-h-16 min-h-16 max-w-16 min-w-16 items-center justify-center rounded-full border text-center text-xs"
                            >
                                No Image
                            </div>
                            ...
                            <div class="flex flex-col gap-1">
                                ...
                                <div class="custom-scrollbar flex max-w-[240px] items-center gap-1 overflow-x-auto">
                                    <span
                                        v-for="attribute in item.attributes"
                                        :key="attribute.id"
                                        class="text-body-muted border-muted bg-content rounded-md border px-2 py-1 text-xs font-medium text-nowrap"
                                    >
