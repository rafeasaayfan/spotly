...
const lastAttributeIndex = ref(0);
...
const handleValueSelected = (attributeIndex: number, variant_ids: [], value: string) => {
    ...
    if (props.product.attributes_values_map.length === valuesSelected.value.length) {
        const selected = props.product.variants.find((variant: any) => {
        ...
        if (selected) {
            emit('changeSelectedVariant', selected);
            return;
        } else {
            emit('changeSelectedVariant', null);
        }
    }

    lastAttributeIndex.value = attributeIndex + 1;

    availableVariantIds.value = [...variant_ids];
...
const recordedValues = ref<{ [key: number]: AttributeValue[] }>({});
const handleAvailableValues = (values: [], attributeIndex: number) => {
    if (!availableVariantIds.value.length) {
        recordedValues.value[attributeIndex] = values;

        return recordedValues.value[attributeIndex];
    }

    if (attributeIndex === lastAttributeIndex.value) {
        const arrayValues: AttributeValue[] = Array.isArray(values) ? values : Object.values(values);

        recordedValues.value[attributeIndex] = arrayValues.filter((value) =>
            value.variant_ids.some((id) => (availableVariantIds.value as any[]).includes(id)),
        );
    }
...
<template>
    <div class="flex flex-col gap-3 pt-4">
        <div
            v-for="(attribute, index) in props.product.attributes_values_map"
            :key="attribute.id"
            class="flex-col gap-1"
            :class="Number(index) <= lastAttributeIndex ? 'flex' : 'hidden'"
        >
            ...
            <div class="flex items-center gap-2">
                <div
                    v-for="value in handleAvailableValues(attribute.values, Number(index))"
                    :key="value.id"
