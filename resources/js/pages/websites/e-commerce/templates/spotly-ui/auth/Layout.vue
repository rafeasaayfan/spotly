...
<template>
    <StyleLayout :colors="reactiveColors">
        <div
            class="relative grid h-screen flex-col items-center justify-center px-4 sm:px-0 lg:max-w-none lg:grid-cols-3 lg:px-0"
            :dir="page.props.lang === 'ar' ? 'rtl' : 'ltr'"
        >
            <div class="web-border-color relative col-span-1 hidden h-full flex-col gap-2 items-center justify-center border-e-5 border-double lg:flex">
                <div
                    :class="page.props.lang === 'ar' ? 'bg-gradient-to-l' : 'bg-gradient-to-r'"
                    class="absolute inset-0 from-[var(--bg_nav_light)] dark:from-[var(--bg_nav_dark)]"
                />
                ...
                <Link href="/" class="z-20">
                    <img
                        ...
                        class="block size-65 rounded-full shadow dark:hidden"
                    />
                    <img
                        v-if="props.websiteNameAndLogo.dark_logo"
                        :src="props.websiteNameAndLogo.dark_logo"
                        class="hidden size-65 rounded-full shadow dark:block"
                    />
                </Link>

                <Link href="/" class="z-10 rounded-md">
                    <span class="web-text-active text-5xl font-bold tracking-widest">{{ props.websiteNameAndLogo.name }}</span>
                </Link>
            </div>
            ...
            <div class="pb-5 lg:p-8 col-span-2">
                <div class="mx-auto flex w-full flex-col justify-center space-y-5 sm:w-[350px]">
                    <Link
                        :href="route('landing')"
                        class="web-bg-body relative z-20 flex h-full items-center justify-center gap-2 rounded-b-md py-1 font-medium lg:hidden"
                    >
                        <img
                            v-if="props.websiteNameAndLogo.light_logo"
                            :src="props.websiteNameAndLogo.light_logo"
