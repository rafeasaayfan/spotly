...
<template>
    <div class="web-border-color border-b pb-2">
        <form @submit.prevent="submit" class="flex items-center gap-3">
            <Button
                variant="ghost"
                type="submit"
                :disabled="loading"
                class="web-bg-content web-text-body-muted size-11 h-11 w-11 rounded-xl p-0 sm:h-11 sm:px-0 sm:py-0"
            >
                <span v-if="loading">
                    <LoaderCircle class="size-5 animate-spin" />
                </span>
                <svg v-if="!loading" class="size-5" enable-background="new 0 0 24 24" version="1.1" viewBox="0 0 24 24">
                    <g>
                        <path
