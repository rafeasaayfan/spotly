...
function submit() {
    ...
    form.post(route('dashboard.attributes.store'), {
        onSuccess: () => {
...
<template>
    <form class="grid grid-cols-1 gap-6 px-4 pb-5 lg:grid-cols-2" @submit.prevent="submit" enctype="multipart/form-data">
        ...
        <div class="flex flex-col gap-1.5">
            <Label>Website</Label>
            <SelectWithSearch v-model="form.website_id" placeholder="Select the website" required :options="mappedWebsites" />
            <InputError :message="form.errors.website_id" />
        </div>
        <!-- Name -->
        <div class="flex flex-col gap-1.5">
            <Label>Name</Label>
